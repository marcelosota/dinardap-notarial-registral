...
public class TramiteDaoEjb extends GenericDaoEjb<Tramite, Long> implements TramiteDao {
    ...
    public List<TramiteRegistradorDto> misTramites(Short contextoNotarial, Short contextoRegistral, Integer usuarioId) {
        ...
        StringBuilder sql = new StringBuilder(" select t.tramite_id, t.codigo, concat(i.nombre,' - ', i.siglas) as institucion, t.fecha_registro, t.fecha_cierre,");
        sql.append(" t.observacion, doc1.ruta as notarial, doc2.ruta as registral, t.descripcion, t.identificacion_requirente, t.nombre_requirente");
        sql.append(" from ec_dinardap_notarial_registral.tramite t");
        sql.append(" INNER JOIN ec_dinardap_seguridad.institucion i ON t.institucion_id = i.institucion_id");
        sql.append(" INNER JOIN ec_dinardap_seguridad.usuario u ON t.registrado_por = u.usuario_id");
        ...
        sql.append(" where t.estado in(").append(EstadoTramiteEnum.INCONSISTENTE.getEstado()).append(",").append(EstadoTramiteEnum.CERRADO.getEstado());
        sql.append(" ,").append(EstadoTramiteEnum.INCONSISTENTE_ATENDIDO.getEstado()).append(")");
        ...
        if (lista != null && lista.size() > 0) {
            for (Object[] fila : lista) {
                ...
                if (fila[6] != null) {
                    ...
                    item.setDocumentoNotarial(true);
                } else {
                    item.setRutaNotarial(null);
                    item.setDocumentoNotarial(false);
                }
                if (fila[7] != null) {
                    item.setRutaRegistral(fila[7].toString());
                    item.setDocumentoRegistral(true);
                } else {
                    item.setRutaRegistral(null);
                    item.setDocumentoRegistral(false);
                }
                if(fila[8] != null)
                	item.setDescripcionNotarial(fila[8].toString());
                else
                	item.setDescripcionNotarial(null);
                if(fila[9] != null)
                	item.setIdentificacionRequirente(fila[9].toString());
                else
                	item.setIdentificacionRequirente(null);
                if(fila[10] != null)
                	item.setNombreRequirente(fila[10].toString());
                else
                	item.setNombreRequirente(null);
